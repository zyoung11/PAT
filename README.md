...
# 保存结果到 ./Result.md 文件
show_result("API测试结果汇总", save_to_md=True)
```
...
```python
show_result(title="测试结果汇总", save_to_md=False)
```

- •`title`: 汇总面板的标题（可选，默认为"测试结果汇总"）
- •`save_to_md`: 是否将结果保存到 `./Result.md` 文件（可选，默认为 `False`）
  - 当设置为 `True` 时，会将测试结果保存到 `./Result.md` 文件
  - 文件包含三部分：测试描述表格、结果统计、API返回的json信息
  - 保存成功后会在终端显示绿色提示信息
...
5. **保存测试结果**: 使用 `save_to_md=True` 参数将测试结果保存到文件，便于后续查看和归档
6. **循环测试**: 在循环中使用动态描述确保每个测试有唯一标识
7. **分组测试**: 使用 `clear_test_results` 进行测试分组和结果管理
